...
def packet_print(
    ...
    if header:
        if tx:
            via_color = "red"
            logit.append(f"[bold red]{header}\u2191[/] " f"[cyan]{name}[/]")
        else:
            via_color = "#1AA730"
            f"[{via_color}]<-[/]"
            logit.append(f"[#1AA730]{header}\u2193[/] " f"[cyan]{name}[/]")
        arrow = f"[{via_color}]\u2192[/]"
    else:
        if tx:
            via_color = "red"
            logit.append(f"[bold red]TX\u2191[/] " f"[cyan]{name}[/]")
        else:
            via_color = "#1AA730"
            f"[{via_color}]<-[/]"
            logit.append(f"[#1AA730]PISS\u2193[/] " f"[cyan]{name}[/]")
        arrow = f"[{via_color}]\u2192[/]"
    ...
    if isinstance(packet, aprsd_core.ThirdPartyPacket):
        ...
        sub_head = (
            f" ([{FROM_COLOR}]{sub_pkt.from_call}[/]{arrow}"
            f"{tmp if tmp else ' '}"
    ...
    elif not isinstance(packet, aprsd_core.AckPacket) and not isinstance(
        packet, aprsd_core.RejectPacket
    ):
        logit.append(" : ")
